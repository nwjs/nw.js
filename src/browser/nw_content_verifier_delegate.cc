...
ContentVerifierDelegate::Mode NWContentVerifierDelegate::GetDefaultMode() {
  ...
  if (command_line->HasSwitch(switches::kVerifyContent)) {
    ...
    if (switch_value == "enforce_strict")
...
const ContentVerifierKey& NWContentVerifierDelegate::PublicKey() {
  static ContentVerifierKey key(kNWSignaturesPublicKey,
...
void NWContentVerifierDelegate::VerifyFailed(
  ...
  LOG(FATAL) << "verification failed: " << relative_path.AsUTF8Unsafe();
}
